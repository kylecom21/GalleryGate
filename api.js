...
const ViteRijksApiKey = import.meta.env.VITE_RIJKS_API_KEY;
...
const searchMetMuseum = async (query) => {
  try {
    ...
    if (!response.data.objectIDs) return [];
...
const searchRijksMuseum = async (query) => {
  try {
    const response = await axios.get(rijksApiUrl, {
      params: { key: ViteRijksApiKey, q: query, format: "json" },
    });
...
const fetchMetArtworks = async (page = 2, limit = 10) => {
  try {
    const response = await axios.get(`${metApiUrl}/search?q=art&hasImages=true`);
    const objectIDs = response.data.objectIDs || [];
    if (!objectIDs.length) return [];

    const paginatedIDs = objectIDs.slice((page - 1) * limit, page * limit);
    const artworkPromises = paginatedIDs.map(async (id) =>
      axios.get(`${metApiUrl}/objects/${id}`).then((res) => res.data).catch(() => null)
    );

    const artworks = await Promise.allSettled(artworkPromises);
    return artworks.filter((art) => art.status === "fulfilled" && art.value?.primaryImage).map((art) => art.value);
  } catch (error) {
...
const fetchRijksArtworks = async (page = 1, limit = 10) => {
  try {
    const response = await axios.get(
      `${rijksApiUrl}?key=${ViteRijksApiKey}&ps=${limit}&p=${page}`
    );
...
const fetchRijksArtworkDetails = async (objectNumber) => {
  try {
    const response = await axios.get(
      `${rijksApiUrl}/${objectNumber}?key=${ViteRijksApiKey}`
    );
